...
import { TIPOS_EXAME, type TipoExame, RISCO_LABEL, RISCO_COR } from '../lib/risco'
...
export default function StatusPage() {
  ...
  const risco = s.classificacao_risco as 'baixo'
  return (
    <PlatformShell title={`Status — ${s.nome}`} subtitle={s.evento_nome ?? undefined} back={{ to: '/destemidos-pioneiros', label: 'Site' }}>
      <div className="dp-card" style={{ display: 'flex', gap: '2rem', flexWrap: 'wrap', marginBottom: '1rem' }}>
        ...
        <div><div style={{ fontSize: '0.8rem', color: 'var(--dp-n-500)' }}>Risco</div><div className="dp-display" style={{ fontSize: '1.4rem', color: RISCO_COR[risco] }}>{RISCO_LABEL[risco] ?? s.classificacao_risco}</div></div>
      </div>
